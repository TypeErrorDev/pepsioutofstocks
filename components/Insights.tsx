...
export default function Insights() {
  ...
  const analytics = useMemo(() => {
    ...
    return {
      stores: Object.values(storeMap).sort(
        (a, b) => b.stockouts + b.gaps - (a.stockouts + a.gaps),
      ),
      chartData,
      total: logs.length,
    };
  ...
  if (!analytics)
    return (
      <div className="min-h-screen bg-app-bg flex items-center justify-center text-app-muted uppercase font-black">
        No Data Available
      </div>
    );
    ...
    <div className="min-h-screen bg-app-bg p-4 md:p-10 transition-colors duration-300">
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex items-center gap-4">
          <Link
            href="/"
            className="p-3 bg-app-card border border-app-border rounded-2xl text-app-muted hover:text-pepsi-blue transition-all"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
        ...
        <div className="bg-app-card border border-app-border rounded-[2.5rem] overflow-hidden shadow-xl">
          ...
          <div className="divide-y divide-app-border">
            {analytics.stores.map((store) => (
              <div
                key={store.name}
                className="p-6 flex items-center justify-between hover:bg-app-bg/50 transition-colors"
              >
                <div>
